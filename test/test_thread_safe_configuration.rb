...
class TestThreadSafeConfiguration < Minitest::Test
  ...
  def test_thread_safe_configuration_isolation
    ...
    # Verify isolation
    assert_equal true, results[0][:enabled], "Thread 0 should have enabled=true"
    assert_equal true, results[0][:debug_logging], "Thread 0 should have debug_logging=true"
    assert_equal false, results[1][:enabled], "Thread 1 should have enabled=false"
    assert_equal false, results[1][:debug_logging], "Thread 1 should have debug_logging=false"
  end
  ...
  def test_nested_configuration_overrides
    ...
    OutboundHttpLogger.with_configuration(enabled: true, debug_logging: false) do
      assert_equal true, OutboundHttpLogger.configuration.enabled?
      assert_equal false, OutboundHttpLogger.configuration.debug_logging

      OutboundHttpLogger.with_configuration(debug_logging: true) do
        assert_equal true, OutboundHttpLogger.configuration.enabled?
        assert_equal true, OutboundHttpLogger.configuration.debug_logging
      end
  ...
  def test_configuration_backup_and_restore
    ...
    expected_keys = [:enabled, :excluded_urls, :excluded_content_types, :sensitive_headers,
    ...
    refute_equal 99999, config.max_body_size
  end
